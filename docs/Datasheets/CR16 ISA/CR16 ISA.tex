...
% Comment if compiling for 'Lab 4 Report.tex'
\begin{center}
...
\end{center}
% Uncomment if compiling for 'Lab 4 Report.tex'
% \begin{center}
% \LARGE{\textbf{Appendix}}\\[7pt]
% \Large{CompactRISC16 (CR16) Instruction Set Architecture (ISA)}
% \end{center}
...
LPC & Rdest & \code{Rdest = PC + 1} & 1111 & Rdest & 0101 & xxxx & Sets Rdest to \code{PC + 1} & 3 \\ \hline
...
\begin{FlushLeft}
\small
Note that during the execution cycle of an instruction, PC (the ``Program Counter'') always points to the next instruction (e.g. \code{PC + 1}). As a result of this, \code{B[condition]} and \code{CALLD} will displace the current PC by \code{Imm + 1}. This can be thought of as executing the ``next instruction'' after PC displacement. Similarly, \code{LPC Rdest} loads \code{PC + 1} into \code{Rdest}.
\end{FlushLeft}
